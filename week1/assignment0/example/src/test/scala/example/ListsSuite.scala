 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("max of empty List throws exception") {
    intercept[NoSuchElementException] {
      max(List())
    }
  }

  test("max of List including negative numbers") {
    assert(max(List(2,4,-6)) === 4)
  }

  test("max of List containing equal elements") {
